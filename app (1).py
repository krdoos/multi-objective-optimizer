...
if uploaded_file is not None:
    ...
    if st.button("🚀 Run Optimization", type="primary"):
        with st.spinner("Processing data and optimizing..."):
            ...
            # إذا تم اختيار نوع صخر وتم حساب UCS، قم بإضافة نسبة MSE/UCS
            try:
                # ucs_psi يتم تعريفه في الشريط الجانبي، إذا لم يكن موجوداً سيكون None
                if 'ucs_psi' in locals() and ucs_psi:
                    df['MSE_ratio'] = df['MSE_psi'] / ucs_psi
            except Exception:
                pass
            ...
            for i, group_name in enumerate(groups):
                ...
                # Founder point: where the slope of the ROP curve transitions from positive to zero.
                ...
                # -------------------------------------------------------------------------
                # Generate fine WOB values for optimization.  If a rock type is selected
                # and a recommended WOB range is available in the rock_properties
                # dictionary, restrict the search to the overlap of this range and the
                # observed WOB range in the current group.  This ensures that the
                # optimization respects geologic constraints and avoids unrealistic
                # extrapolation.
                # -------------------------------------------------------------------------
                wob_rec_range = None
                try:
                ...
                if wob_rec_range and isinstance(wob_rec_range, (list, tuple)) and len(wob_rec_range) == 2:
                    ...
                    # Constrain search to the intersection of recommended and data ranges
                    wob_search_min = max(agg['WOB_klb'].min(), rec_low)
                    wob_search_max = min(agg['WOB_klb'].max(), rec_high)
                    ...
                    # Use full observed range if no recommendation available
                    wob_fine = np.linspace(agg['WOB_klb'].min(), agg['WOB_klb'].max(), 200)

                # Evaluate polynomial fits at fine WOB values
                ...
                # Compute desirability across the fine WOB grid and locate the optimum
                des_fine = np.array([
                ...
                # -------------------------------------------------------------------------
                # Calculate MSE at founder and optimal points and compute ratios to the
                # formation's UCS (if provided).  Ratios near unity indicate that the
                # drilling energy matches the rock strength, achieving maximum efficiency
                # according to literature【436714205699149†L204-L217】【612207715040690†L90-L101】.
                # -------------------------------------------------------------------------
                try:
                    founder_mse_psi = float(poly_mse(founder_wob_klb))
                except Exception:
                    founder_mse_psi = np.nan
                if 'ucs_psi' in locals() and ucs_psi:
                    founder_mse_ratio = founder_mse_psi / ucs_psi
                    opt_mse_ratio = mse_opt_psi / ucs_psi
                else:
                ...
                # حفظ النتائج، بما في ذلك نسب MSE/UCS عند نقطتي المؤسس والمثلى
                results.append({
                ...
                if wob_rec_range and isinstance(wob_rec_range, (list, tuple)):
                    caption_parts.append(
                        f"The search for optimal WOB was restricted to the recommended range for {rock_type} (\​{wob_rec_range[0]:.1f}–{wob_rec_range[1]:.1f} klb)."
                    )
