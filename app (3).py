...
def calc_mse(wob_lbs, rpm, tob_ftlb, rop_fthr, bit_area_in2):
    rop_safe = np.where(np.array(rop_fthr) <= 0, 0.001, np.array(rop_fthr))
    term1 = np.array(wob_lbs) / bit_area_in2
    term2 = (120.0 * np.pi * np.array(rpm) * np.array(tob_ftlb)) / (bit_area_in2 * rop_safe)
    return term1 + term2
...
if st.session_state.opt_done and st.session_state.results_df is not None:
    ...
    cmap = plt.cm.get_cmap('tab10', len(layers_in_data))
